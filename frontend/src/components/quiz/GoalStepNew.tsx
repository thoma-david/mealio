...
const GoalStep: React.FC<GoalStepProps> = ({ data, onUpdate }) => {
  const goals = [
    ...
    },
    {
      value: 'save_money',
      label: 'Save Money',
      description: 'Affordable, budget-friendly meals without compromising nutrition',
        ...
        >
          {goals.map((goal) => (
            <Paper
              ...
              elevation={0}
              sx={{
                border: `2px solid ${data.goal === goal.value ? '#ff7043' : alpha('#000', 0.1)}`,
                borderRadius: 3,
                transition: 'all 0.3s ease',
                cursor: 'pointer',
                bgcolor: data.goal === goal.value ? alpha('#ff7043', 0.05) : 'transparent',
                '&:hover': {
        ...
        <Box sx={{ mt: 4 }}>
          ...
          <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
            <TextField
              ...
              onChange={(e) => onUpdate({ goalWeight: Number(e.target.value) || undefined })}
              InputProps={{
            ...
            <TextField
              ...
              onChange={(e) => onUpdate({ goalTimeframe: Number(e.target.value) })}
              fullWidth
          ...
          {data.goalWeight && data.weight && (
            <Box sx={{ mt: 2, p: 2, bgcolor: alpha('#ff7043', 0.05), borderRadius: 2 }}>
              <Typography variant="body2" color="text.secondary">
                <strong>Target:</strong> {data.goalType === 'lose' ? 'Lose' : 'Gain'}{' '}
                {Math.abs(data.goalWeight - data.weight).toFixed(1)} kg{' '}
                {data.goalTimeframe && `in ${data.goalTimeframe} month${data.goalTimeframe > 1 ? 's' : ''}`}
                {data.goalTimeframe && (
                  <span>
                    {' '}(~{(Math.abs(data.goalWeight - data.weight) / data.goalTimeframe).toFixed(1)} kg/month)
                  </span>
                )}
              </Typography>
            </Box>
